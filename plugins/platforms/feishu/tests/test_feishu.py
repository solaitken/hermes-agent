...
class TestFeishuMessageNormalization(unittest.TestCase):
    def test_normalize_merge_forward_preserves_summary_lines(self):
        from hermes_agent_feishu.adapter import normalize_feishu_message
...
class TestFeishuAdapterMessaging(unittest.TestCase):
    ...
    def test_connect_webhook_mode_starts_local_server(self):
        ...
        with (
            patch("hermes_agent_feishu.adapter.FEISHU_AVAILABLE", True),
            patch("hermes_agent_feishu.adapter.FEISHU_WEBHOOK_AVAILABLE", True),
            patch("hermes_agent_feishu.adapter.EventDispatcherHandler") as mock_handler_class,
            patch("hermes_agent_feishu.adapter.acquire_scoped_lock", return_value=(True, None)),
            patch("hermes_agent_feishu.adapter.release_scoped_lock"),
            ...
            patch("hermes_agent_feishu.adapter.web", web_module),
        ):
    ...
    def test_connect_acquires_scoped_lock_and_disconnect_releases_it(self):
        ...
        with (
            patch("hermes_agent_feishu.adapter.FEISHU_AVAILABLE", True),
            patch("hermes_agent_feishu.adapter.FEISHU_WEBSOCKET_AVAILABLE", True),
            patch("hermes_agent_feishu.adapter.lark", SimpleNamespace(LogLevel=SimpleNamespace(INFO="INFO", WARNING="WARNING"))),
            patch("hermes_agent_feishu.adapter.EventDispatcherHandler") as mock_handler_class,
            patch("hermes_agent_feishu.adapter.FeishuWSClient", return_value=ws_client),
            patch("hermes_agent_feishu.adapter._run_official_feishu_ws_client"),
            patch("hermes_agent_feishu.adapter.acquire_scoped_lock", return_value=(True, None)) as acquire_lock,
            patch("hermes_agent_feishu.adapter.release_scoped_lock") as release_lock,
            patch.object(adapter, "_hydrate_bot_identity", new=AsyncMock()),
            patch.object(adapter, "_build_lark_client", return_value=SimpleNamespace()),
        ):
            _mock_event_dispatcher_builder(mock_handler_class)
            ...
            try:
                with patch("hermes_agent_feishu.adapter.asyncio.get_running_loop", return_value=_Loop()):
                    connected = asyncio.run(adapter.connect())
    ...
    def test_connect_rejects_existing_app_lock(self):
        ...
        with (
            ...
            patch("hermes_agent_feishu.adapter.FEISHU_WEBSOCKET_AVAILABLE", True),
            patch(
                "hermes_agent_feishu.adapter.acquire_scoped_lock",
    ...
    def test_connect_retries_transient_startup_failure(self):
        ...
        with (
            ...
            patch("hermes_agent_feishu.adapter.release_scoped_lock"),
            patch.object(adapter, "_hydrate_bot_identity", new=AsyncMock()),
            patch("hermes_agent_feishu.adapter.asyncio.sleep", side_effect=lambda delay: sleeps.append(delay)),
            patch.object(adapter, "_build_lark_client", return_value=SimpleNamespace()),
        ):
            _mock_event_dispatcher_builder(mock_handler_class)
            ...
            fake_loop = _Loop()
            try:
                with patch("hermes_agent_feishu.adapter.asyncio.get_running_loop", return_value=fake_loop):
                    connected = asyncio.run(adapter.connect())
            finally:
                loop.close()
...
class TestAdapterModule(unittest.TestCase):
    ...
    def test_runtime_ws_overrides_reapply_after_sdk_configure(self):
        ...
        try:
            from hermes_agent_feishu.adapter import _run_official_feishu_ws_client
...
class TestAdapterBehavior(unittest.TestCase):
    ...
    def test_build_event_handler_registers_reaction_and_card_processors(self):
        ...
        with patch("hermes_agent_feishu.adapter.EventDispatcherHandler", _Dispatcher):
    ...
    def test_bot_origin_reactions_are_dropped_to_avoid_feedback_loops(self):
        ...
        for emoji in ("Typing", "CrossMark"):
            ...
            with patch(
                "hermes_agent_feishu.adapter.asyncio.run_coroutine_threadsafe"
            ) as run_threadsafe:
    ...
    def test_message_event_submits_to_adapter_loop(self):
        ...
        with patch("hermes_agent_feishu.adapter.asyncio.run_coroutine_threadsafe", side_effect=_submit) as submit:
    ...
    def test_text_batch_merges_rapid_messages_into_single_event(self):
        ...
        async def _run() -> None:
            with patch("hermes_agent_feishu.adapter.asyncio.sleep", side_effect=_sleep):
    ...
    def test_send_image_downloads_then_uses_native_image_send(self):
        ...
        async def _run():
            with patch("hermes_agent_feishu.adapter.cache_image_from_url", new=AsyncMock(return_value="/tmp/cached.png")):
                ...
                async def _run() -> tuple[str, str]:
                    with patch("tools.url_safety.is_safe_url", return_value=True):
                        with patch("httpx.AsyncClient", _FakeAsyncClient):
                            with patch(
                                "hermes_agent_feishu.adapter.cache_document_from_bytes",
    ...
    def test_send_retries_transient_failure(self):
        ...
        with (
            patch("hermes_agent_feishu.adapter.asyncio.to_thread", side_effect=_direct),
            patch("hermes_agent_feishu.adapter.asyncio.sleep", side_effect=_sleep),
        ):
...
class TestPendingInboundQueue(unittest.TestCase):
    ...
    def test_event_queued_when_loop_not_ready(self):
        ...
        with patch("hermes_agent_feishu.adapter.threading.Thread") as thread_cls:
    ...
    def test_drainer_replays_queued_events_when_loop_becomes_ready(self):
        ...
        with patch("hermes_agent_feishu.adapter.threading.Thread"):
            for ev in events:
    ...
    def test_normal_path_unchanged_when_loop_ready(self):
        ...
        with patch(
            ...
            "hermes_agent_feishu.adapter.threading.Thread"
        ) as thread_cls:
...
class TestWebhookSecurity(unittest.TestCase):
    ...
    def test_rate_limit_blocks_after_exceeding_max(self):
        from hermes_agent_feishu.adapter import _FEISHU_WEBHOOK_RATE_LIMIT_MAX
        adapter = self._make_adapter()
    ...
    def test_rate_limit_resets_after_window_expires(self):
        from hermes_agent_feishu.adapter import _FEISHU_WEBHOOK_RATE_LIMIT_MAX, _FEISHU_WEBHOOK_RATE_WINDOW_SECONDS
        adapter = self._make_adapter()
        ip = "10.0.0.3"
        for _ in range(_FEISHU_WEBHOOK_RATE_LIMIT_MAX):
    ...
    def test_webhook_request_rejects_oversized_body(self):
        from gateway.config import PlatformConfig
        from hermes_agent_feishu.adapter import FeishuAdapter, _FEISHU_WEBHOOK_MAX_BODY_BYTES
...
class TestDedupTTL(unittest.TestCase):
    ...
    def test_expired_entry_is_not_considered_duplicate(self):
        from gateway.config import PlatformConfig
        from hermes_agent_feishu.adapter import FeishuAdapter, _FEISHU_DEDUP_TTL_SECONDS
...
class TestFeishuMentionMap(unittest.TestCase):
    ...
    def test_build_mentions_map_marks_self_by_open_id(self):
        from hermes_agent_feishu.adapter import _build_mentions_map, _FeishuBotIdentity
...
class TestFeishuMentionHint(unittest.TestCase):
    def test_hint_single_user(self):
        from hermes_agent_feishu.adapter import FeishuMentionRef, _build_mention_hint
...
class TestFeishuStripLeadingSelf(unittest.TestCase):
    def _make_refs(self, *, self_name="Hermes", other_name=None):
        from hermes_agent_feishu.adapter import FeishuMentionRef
    ...
    def test_strips_leading_self(self):
        from hermes_agent_feishu.adapter import _strip_edge_self_mentions
    ...
    def test_returns_input_when_no_self_refs(self):
        from hermes_agent_feishu.adapter import _strip_edge_self_mentions, FeishuMentionRef
...
class TestFeishuNormalizeText(unittest.TestCase):
    def test_renders_mention_with_display_name(self):
        from hermes_agent_feishu.adapter import _normalize_feishu_text, FeishuMentionRef
...
class TestFeishuPostMentionParsing(unittest.TestCase):
    def test_post_at_tag_renders_via_mentions_map(self):
        ...
        from hermes_agent_feishu.adapter import parse_feishu_post_payload, FeishuMentionRef
    ...
    def test_post_at_tag_falls_back_to_inline_user_name_when_map_misses(self):
        ...
        from hermes_agent_feishu.adapter import parse_feishu_post_payload
...
class TestFeishuNormalizeWithMentions(unittest.TestCase):
    def test_text_message_renders_mention_by_name(self):
        from hermes_agent_feishu.adapter import normalize_feishu_message, _FeishuBotIdentity
